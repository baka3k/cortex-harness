//! Port phần pipeline của `tools/swift/swift_analyzer.py`: `_scan_swift_files`,
...
use crate::swiftparse::{self, FilePayload, Row};

/// `_SCAN_SKIP_DIRS` của swift_analyzer (union COMMON_SCAN_EXCLUDE tại scan).
...
/// `_scan_swift_files` — walk root, prune skip dirs, thu `.swift`, sorted
/// theo full path; `selected` non-empty ⇒ lọc theo rel path.
pub fn scan_swift_files(root: &Path, selected: &BTreeSet<String>) -> Vec<PathBuf> {
...
fn walk(dir: &Path, files: &mut BTreeSet<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        if swiftparse::SWIFT_SOURCE_EXTENSIONS
            .iter()
...
pub fn build_payloads(
    ...
    let files = scan_swift_files(root, selected);
    let mut payloads = Vec::with_capacity(files.len());
    for path in files {
        payloads.push(swiftparse::parse_swift_file(&path, root)?);
    }
    Ok(payloads)
}

/// Manifest paths (incremental) — sorted, lọc theo extension nguồn.
pub fn manifest_swift_paths(
    manifest: &str,
    root: &Path,
    extensions: &[&str],
) -> BTreeSet<String> {
...
/// `_prepare_write_rows` — swift gom mọi collection qua 1 loop với
/// `row["id"] = pop("symbol_id")`; shapes dưới đây khớp key-by-key.
    ...
    for payload in payloads {
        ...
        for item in &payload.calls {
            if item.callee_id.is_some() {
                ...
                // supply trực tiếp (tiền lệ analyzer-js / analyzer-rust).
...
pub struct PipelineConfig {
    ...
    /// `_selected_rel_paths` (đã lọc .swift).
    pub cleanup_selected: BTreeSet<String>,
    /// `_deleted_rel_paths` (đã lọc .swift).
