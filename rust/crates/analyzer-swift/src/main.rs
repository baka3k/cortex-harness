...
fn finish(
    ...
    // embedding-input artifact (the embedding pass runs with the store
    // closed, so this cannot live inside the graph-disabled branch).
    ...
    if let Some(output) = args.embedding_input_output()
        ...
        {
            eprintln!("Swift embedding-input artifact failed: {error}");
            return 3;
        }
