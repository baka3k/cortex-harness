//! `KotlinAnalyzer` — pipeline `build_call_graph` của kotlin_analyzer.py:
//! scan (skip-list kotlin riêng) → import-impact selection → cleanup → parse
//! (tree-sitter-kotlin) → index → resolve → `LanguageCodeWriter::write_all`
//! → `[SCAN_RESULT]`.
//!
...
use crate::kotlin_parse::{parse_kotlin_file, FilePayload};
use crate::kotlin_scan::{
    collect_kotlin_import_graph, expand_impacted_files_by_imports, scan_kotlin_files,
};
use crate::resolve::{assemble_graph, ClassIndex, FunctionIndex};

/// Cờ kotlin-specific ngoài contract chung — mirror `parse_args` của
/// kotlin_analyzer.py; phần không áp dụng cho backend Rust nhận và bỏ qua.
#[derive(Debug, Clone, Default, clap::Args)]
pub struct KotlinExtraArgs {
    #[arg(long, hide = true)]
...
pub fn execute(args: &AnalyzerArgs, extra: &KotlinExtraArgs) -> Result<i32, String> {
    ...
    // ── Dry run ─────────────────────────────────────────────────────────────
    if args.dry_run {
        let mut files = scan_kotlin_files(&root);
        if args.incremental && !changed_files.is_empty() {
            let all_rel_paths: Vec<String> = files
                .iter()
            ...
            let changed_existing: HashSet<String> = changed_files
                ...
                .cloned()
                .collect();
            let deps_by_file = collect_kotlin_import_graph(&files, &root);
            ...
            files = all_rel_paths
                ...
                .collect();
            println!(
                "Dry run (incremental): {} Kotlin files selected (manifest={} impacted={})",
                files.len(),
                changed_files.len(),
                impacted.len()
            );
    ...
    let language = non_empty(args.language.clone()).unwrap_or_else(|| "kotlin".to_string());
    ...
    // ── Scan ────────────────────────────────────────────────────────────────
    let all_scanned_files = scan_kotlin_files(&root);
    ...
    if incremental {
        let changed_existing: HashSet<String> = changed_set
            ...
            .cloned()
            .collect();
        let deps_by_file = collect_kotlin_import_graph(&all_scanned_files, &root);
        let impacted = expand_impacted_files_by_imports(&changed_existing, &deps_by_file);
        let mut selected = BTreeSet::new();
        selected.extend(changed_existing.iter().cloned());
        selected.extend(impacted.iter().cloned());
        ...
        kotlin_files = all_scanned_files.clone();
        impacted_count = 0;
    }
    if verbose {
        if incremental {
    ...
    if let Some(store) = store.take() {
        if verbose {
            println!("[graph] Writing nodes and relations (batch collection)...");
            println!("[graph] Creating project node...");
            println!("[graph] Collecting entities from payloads...");
        }
        ...
        if verbose {
            println!(
                "[graph] Collected {} packages, {} namespaces, {} files, {} classes, {} function_types, {} functions, {} relations, {} calls",
                graph.packages.len(),
                graph.namespaces.len(),
                graph.files.len(),
                graph.classes.len(),
                graph.function_types.len(),
                graph.functions.len(),
                graph.relations.len(),
                graph.calls.len(),
            );
