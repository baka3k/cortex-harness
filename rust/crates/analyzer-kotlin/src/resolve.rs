//! Port `build_call_graph` phần graph write của kotlin_analyzer.py: function/
//! class index từ index_payloads (toàn bộ file khi incremental), external
//! classes cho type-edge target không resolve, JSON rows cho `write_all`.

use serde_json::{json, Map, Value};

use crate::kotlin_parse::{CallEdge, FilePayload, Row};
...
/// Kết quả resolve target của type edge — khớp logic Python.
...
fn relation_row(source_id: &str, target_id: &str, rel_type: &str, properties: Row) -> Row {
    let mut row = Map::new();
...
fn build_note(code: &str, comment: &str, summary: &str) -> String {
    crate::kotlin_parse::build_note(code, comment, summary)
}
...
pub fn assemble_graph(
    ...
    // Pass 1: files/packages/namespaces/classes/function_types/functions.
    for payload in selected_payloads {
        ...
        file_row.insert("file_path".into(), json!(file_id));
        ...
        if let Some(package_def) = &payload.package_def {
            ...
            if !seen_namespaces.contains(&namespace_id) {
                ...
                ns_row.insert("summary".into(), json!(namespace_summary));
    ...
    // Pass 2: relations + calls (python quét selected_payloads lần 2).
    for payload in selected_payloads {
        let file_def = payload.file_def.as_ref().expect("file_def");
        let file_id = file_def.file_path.clone();
        ...
        for func in &payload.functions {
            ...
            if let Some(class_name) = &func.class_name {
                let class_id = crate::kotlin_parse::class_id(func.package_name.as_deref(), class_name);
                graph.relations.push(relation_row(
                    &class_id,
                    &func.symbol_id,
                    "DECLARES",
        ...
        // Type edges (inheritance) — target resolve hoặc fallback target_name.
        for edge in &payload.type_edges {
            let target_name = &edge.target_name;
            let resolved = class_index.resolve_target(target_name, edge.source_package.as_deref());
        ...
        // Custom relations (TAKES_FUNCTION, lambda CONTAINS).
        ...
        // Calls — callee_id resolve qua function index.
        for call in &payload.calls {
            let callee_id = function_index.resolve_callee_id(call);
                ...
                && !callee_id.is_empty()
            {
                ...
                call_row.insert("call_type".into(), json!("direct"));
