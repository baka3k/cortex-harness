...
Follow-up Windows validation found `tools.servlet_jsp.pipeline` imported POSIX-only `resource` at module load time, which prevented `tools.servlet_jsp.servlet_jsp_analyzer` from importing on Windows. The import is now optional and peak-RSS checks are skipped only when the platform has no `resource` module. The MyBatis runtime dependency on `tree-sitter-language-pack` is now explicit in both requirements files, and the previously ignored framework tests plus `tests/fixtures/framework-java-app` are tracked so the validation evidence is reproducible.
