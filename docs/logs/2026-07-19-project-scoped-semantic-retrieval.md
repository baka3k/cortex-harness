...
- Explore falls back to the proven semantic backend embedder when `sentence-transformers` cannot load a remote-code model such as Jina v3, so scoped exploration reaches Qdrant instead of silently returning no candidates (`code-tiny/mcp/services/explore_service.py:90`).
