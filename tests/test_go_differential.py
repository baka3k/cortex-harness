#!/usr/bin/bin/python
"""Differential test: Rust cortex_extract.extract_go vs Python parse_go_file.

Runs both extractors on the same Go fixture files and compares the output
payloads field by field. Core fields (functions, calls, fields, types,
namespaces, includes, using_namespaces, using_imports) must match exactly.

Known divergence: Rust extracts external types from field type signatures
(e.g. generic type param `T` in `Store[T any]`) more faithfully than the
Python analyzer. This is a Python-side inconsistency in `_add_type_use`
propagation during `_walk_tree` recursion, not a Rust bug. External types
...
FIXTURE_DIR = os.path.join(_ROOT, "tests", "fixtures", "go-app")
...
def run_differential():
    ...
    if not go_files:
        print("ERROR: No .go fixture files found in", FIXTURE_DIR)
        return 1

    print(f"Found {len(go_files)} Go fixture files")
    all_good = True

    for go_file in go_files:
        rel = os.path.relpath(go_file, FIXTURE_DIR)
        print(f"\n{'='*60}")
        print(f"Testing: {rel}")
        print(f"{'='*60}")

        try:
            py_payload = parse_go_file(go_file, FIXTURE_DIR)
        ...
        try:
            rust_payload = cortex_extract.extract_go(go_file, FIXTURE_DIR)
        except Exception as e:
        ...
        # Types (excluding external — Python-side inconsistency)
        rust_real = {t["name"]: t for t in rust_payload["types"] if t["kind"] != "external"}
        py_types = {t["name"]: t for t in py_payload["types"]}
        ...
        # Container fields
        for key in ("includes", "using_namespaces", "using_imports"):
