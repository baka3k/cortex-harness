...
use crate::swift_lang::SwiftParseOutput;
...
/// Build a Python `dict` payload from a `SwiftParseOutput`.
///
/// Swift (like Rust-lang / Go) is a Family A — list-typed — schema; the
/// output matches `parse_swift_file()`'s dict exactly.
pub fn build_swift_payload(py: Python, out: &SwiftParseOutput) -> PyResult<PyObject> {
    let dict = PyDict::new(py);
    ...
    // namespaces (Swift emits no NamespaceDefs, so always empty)
    dict.set_item("namespaces", PyList::empty(py))?;
    ...
    // function_types (always empty for Swift)
    dict.set_item("function_types", PyList::empty(py))?;
    ...
    // scalar list fields
    dict.set_item("using_namespaces", PyList::new(py, out.using_namespaces.iter()))?;
    dict.set_item("using_imports", PyList::new(py, out.using_imports.iter()))?;
    dict.set_item("includes", PyList::new(py, out.includes.iter()))?;
    dict.set_item("macros", PyList::new(py, out.macros.iter()))?;
