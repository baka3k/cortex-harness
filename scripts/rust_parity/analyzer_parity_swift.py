#!/usr/bin/env python3
"""Phase 07 parity gate — swift analyzer Python vs Rust.

Dual-run `tools/swift/swift_analyzer.py` và `analyzer-swift` trên 2 graph
FalkorDB riêng (FULL + incremental với changed/deleted manifests), dump và
so exact ngoài mask chuẩn.

Lưu ý invocation:
* swift_analyzer.py KHÔNG nhận `--config` nên phía Python bỏ cờ này; backend
  Rust accept-and-ignore qua framework AnalyzerArgs.
* CALLS rows của swift_analyzer.py không mang project_id ⇒ reference CHỈ chạy
  được qua đúng đường invocation production: orchestrator cấp journal env
  (`configure_journal_env`, mode `shadow` = không journal runtime/write/guard,
  writer chỉ đọc metadata.project_id làm fallback cho call rows). Rust supply
  project_id tường minh trên call rows — cùng graph.
* swift_analyzer.py KHÔNG có import-impact expansion: incremental chọn đúng
  manifest changed (lọc .swift) — cleanup = sorted(selected ∪ deleted).

Usage (từ repo root):
    .venv/bin/python scripts/rust_parity/analyzer_parity_swift.py
"""
...
TESTDATA = REPO / "tests" / "fixtures" / "swift-analyzer"
PY_BIN = REPO / ".venv" / "bin" / "python"
PY_ANALYZER = REPO / "code-tiny" / "tools" / "swift" / "swift_analyzer.py"
RUST_BIN = REPO / "rust" / "target" / "release" / "analyzer-swift"
REPORT_PATH = (
    REPO / "plans" / "260913-2130-rust-full-migration" / "reports"
    / "phase07-swift-parity.md"
)
...
# `_SCAN_SKIP_DIRS` của swift_analyzer — dùng để đếm corpus .swift của stock.
SWIFT_SKIP_DIRS = {
    ".git", ".hg", ".svn",
    ".idea", ".vs", ".vscode", ".eclipse", ".settings",
    ".build", "build", "out", "bin", "obj",
    "DerivedData", ".swiftpm", "Packages",
...
def journal_env(env: dict, root: Path, project_id: str, graph: str, host: str, port: int) -> dict:
    ...
    trên call rows (hoặc journal metadata fallback). swift_analyzer.py dựng
    rows không project_id nên CHỈ chạy được qua đúng đường invocation
    production: orchestrator cấp journal env cho child process
    (`configure_journal_env`). Mode `shadow` ⇒ `required=False`: KHÔNG có
    journal runtime/write/guard — writer chỉ đọc `metadata.project_id` làm
    fallback cho call rows, cho ra đúng graph như khi rows mang project_id
    tường minh (Rust supply trực tiếp giá trị này).
    """
    journal_cache = Path(tempfile.mkdtemp(prefix="p07_swift_journal_"))
...
def dual(driver: FalkorDBDriver, root: Path, tag: str, host: str, port: int,
    ...
    graph_py = f"p07_swift_{tag}_py"
    graph_rust = f"p07_swift_{tag}_rs"
    if clean:
        clean_graph(driver, graph_py)
        clean_graph(driver, graph_rust)
    py_log = run_py(root, "parity_swift", graph_py, host, port, incremental)
    rust_log = run_rust(root, "parity_swift", graph_rust, host, port, incremental)
...
def scenario_incremental(driver: FalkorDBDriver, host: str, port: int, report: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="p07_swift_inc_") as tmp:
        workdir = Path(tmp) / "corpus"
        workdir.mkdir()
        for name in ("models.swift", "networking.swift", "app.swift"):
        ...
        # Mutate: sửa networking.swift (thêm function), thêm file, xoá app.swift.
        # swift_analyzer KHÔNG có import-impact expansion ⇒ selected đúng bằng
        # manifest changed (networking.swift + parity_added.swift).
        (workdir / "networking.swift").write_text(
            (workdir / "networking.swift").read_text(encoding="utf-8")
            + "\nfunc appendedParityHelper(values: [Int]) -> Int {\n"
            + "    return values.reduce(0, +)\n"
            + "}\n",
            encoding="utf-8",
        )
        added = workdir / "parity_added.swift"
        added.write_text(
            "// added for parity incremental test\n"
            "func brandNewFunction(seed: Double) -> Double {\n"
            "    return seed * 2.0\n"
            "}\n",
            encoding="utf-8",
        )
        (workdir / "app.swift").unlink()

        changed_manifest = Path(tmp) / "changed.json"
        deleted_manifest = Path(tmp) / "deleted.json"
        changed_manifest.write_text(
            json.dumps({"files": ["networking.swift", "parity_added.swift"]}) + "\n",
            encoding="utf-8",
        )
        deleted_manifest.write_text(
            json.dumps({"files": ["app.swift"]}) + "\n", encoding="utf-8"
        )
        # KHÔNG clean giữa seed và incremental — cleanup phải xoá node thật
        # (File + functions/types của app.swift) khỏi graph seed.
        dual(driver, workdir, "inc_run", host, port, report,
             incremental=(changed_manifest, deleted_manifest), clean=False)


def count_stock_swift(stock: Path) -> int:
    count = 0
    for dirpath, dirnames, filenames in os.walk(stock):
        dirnames[:] = [name for name in dirnames if name not in SWIFT_SKIP_DIRS]
...
def main() -> int:
    ...
    if not RUST_BIN.exists():
        ...
        print("Build first: cargo build --release -p analyzer-swift (run inside rust/)")
        return 1
    ...
    report = [
        ...
        f"- testdata: `{TESTDATA.relative_to(REPO)}` (class/struct/protocol/"
        "enum/extension/methods/fields/typealias/subscript/cross-file)",
    ...
    if not args.skip_stock and STOCK.exists():
        stock_swift = count_stock_swift(STOCK)
        if stock_swift >= 3:
            print(f"[stock] {stock_swift} .swift files scanned (excluding skip dirs)")
            dual(driver, STOCK, "stock_full", args.host, args.port, report)
        else:
            note = (
                f"[warn] skip stock: chỉ {stock_swift} file .swift dưới stock "
                "(ngoài skip dirs) < 3 — không có matching sources"
            )
            print(note)
