#!/usr/bin/env python3
"""Phase 08 parity gate — sql analyzer Python vs Rust.
...
PY_ANALYZER = REPO / "code-tiny" / "tools" / "sql" / "sql_analyzer.py"
RUST_BIN = RUST_DIR / "analyzer-sql"
TESTDATA = REPO / "tests" / "fixtures" / "sql-family" / "sql"
REPORT_PATH = (
    REPO / "plans" / "260913-2130-rust-full-migration" / "reports"
    / "phase08-sql-parity.md"
)
...
def dual(driver: FalkorDBDriver, root: Path, tag: str, host: str, port: int,
         ...
         incremental=None, clean: bool = True, project_id: str = "parity_sql",
         graph_tag: str | None = None) -> None:
    graph_tag = graph_tag or tag
    graph_py = f"p08_sql_{graph_tag}_py"
    graph_rust = f"p08_sql_{graph_tag}_rs"
    if clean:
...
def scenario_incremental(driver: FalkorDBDriver, host: str, port: int,
    ...
    with tempfile.TemporaryDirectory(prefix="p08_sql_inc_") as tmp:
        ...
        # Mutate: thêm function mới vào users.sql (thêm call), thêm file, xoá
        # nightly.sql (functions + calls của nó phải bị cleanup khỏi graph seed).
        users_sql = workdir / "routines/users.sql"
        users_sql.write_text(
            users_sql.read_text(encoding="utf-8")
            + """
create procedure parity_added_routine(p_id int)
begin
  call audit_log(p_id);
...
""",
        ...
        added = workdir / "routines/parity_added.sql"
        added.write_text(
            "-- added for parity incremental test\n"
            "create procedure brand_new_callee(p_id int)\n"
            "begin\n"
            "  call audit_log(p_id);\n"
            "end;\n",
            encoding="utf-8",
        )
        (workdir / "jobs/nightly.sql").unlink()

        changed_manifest = Path(tmp) / "changed.json"
        deleted_manifest = Path(tmp) / "deleted.json"
        changed_manifest.write_text(
            json.dumps({"files": ["routines/users.sql", "routines/parity_added.sql"]}) + "\n",
            encoding="utf-8",
        )
        deleted_manifest.write_text(
            json.dumps({"files": ["jobs/nightly.sql"]}) + "\n", encoding="utf-8"
        )
...
def main() -> int:
    ...
    report = [
        "# Phase 08 — sql analyzer parity (python vs rust)",
        "",
        f"- chạy: {time.strftime('%Y-%m-%d %H:%M:%S')}",
        f"- testdata: `{TESTDATA.relative_to(REPO)}` (create procedure/function "
        "regex scan, call/exec/generic-call/bare-call extraction, builtins "
        "pg_catalog/dbms_, unresolved callees)",
    ...
    with tempfile.TemporaryDirectory(prefix="p08_sql_journal_") as journal_tmp:
