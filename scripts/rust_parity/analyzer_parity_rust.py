#!/usr/bin/env python3
"""Phase 07 parity gate — rust analyzer Python vs Rust.

Dual-run `tools/rust/rust_analyzer.py` và `analyzer-rust` trên 2 graph
FalkorDB riêng (FULL + incremental với changed/deleted manifests), dump và
so exact ngoài mask chuẩn.

Lưu ý invocation:
* rust_analyzer.py KHÔNG nhận `--config` (khác js_analyzer) nên phía Python
  bỏ cờ này; backend Rust accept-and-ignore qua framework AnalyzerArgs.
* CALLS rows của rust_analyzer.py không mang project_id ⇒ reference CHỈ chạy
...
* rust_analyzer.py KHÔNG có import-impact expansion: incremental chọn đúng
  manifest changed (lọc .rs) — cleanup = sorted(selected ∪ deleted).

Usage (từ repo root):
    .venv/bin/python scripts/rust_parity/analyzer_parity_rust.py
"""
...
TESTDATA = REPO / "tests" / "fixtures" / "rust-analyzer"
PY_BIN = REPO / ".venv" / "bin" / "python"
PY_ANALYZER = REPO / "code-tiny" / "tools" / "rust" / "rust_analyzer.py"
RUST_BIN = REPO / "rust" / "target" / "release" / "analyzer-rust"
REPORT_PATH = (
    REPO / "plans" / "260913-2130-rust-full-migration" / "reports"
    / "phase07-rust-parity.md"
)
...
# `_SCAN_SKIP_DIRS` của rust_analyzer — dùng để đếm corpus .rs của stock.
RUST_SKIP_DIRS = {
    ".git", ".hg", ".svn",
...
def journal_env(env: dict, root: Path, project_id: str, graph: str, host: str, port: int) -> dict:
    ...
    trên call rows (hoặc journal metadata fallback). rust_analyzer.py dựng
    ...
    """
    journal_cache = Path(tempfile.mkdtemp(prefix="p07_rust_journal_"))
...
def dual(driver: FalkorDBDriver, root: Path, tag: str, host: str, port: int,
    ...
    graph_py = f"p07_rust_{tag}_py"
    graph_rust = f"p07_rust_{tag}_rs"
    if clean:
        clean_graph(driver, graph_py)
        clean_graph(driver, graph_rust)
    py_log = run_py(root, "parity_rust", graph_py, host, port, incremental)
    rust_log = run_rust(root, "parity_rust", graph_rust, host, port, incremental)
...
def scenario_incremental(driver: FalkorDBDriver, host: str, port: int, report: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="p07_rust_inc_") as tmp:
        workdir = Path(tmp) / "corpus"
        workdir.mkdir()
        for name in ("geometry.rs", "container.rs", "service.rs"):
        ...
        # Mutate: sửa container.rs (thêm function), thêm file, xoá service.rs.
        # rust_analyzer KHÔNG có import-impact expansion ⇒ selected đúng bằng
        # manifest changed (container.rs + service_added.rs).
        (workdir / "container.rs").write_text(
            (workdir / "container.rs").read_text(encoding="utf-8")
            + "\npub fn appended_parity_helper(values: &[i64]) -> i64 {\n"
            + "    values.iter().sum::<i64>()\n"
            + "}\n",
            encoding="utf-8",
        )
        added = workdir / "service_added.rs"
        added.write_text(
            "// added for parity incremental test\n"
            "pub fn brand_new_function(seed: f64) -> f64 {\n"
            "    seed * 2.0\n"
            "}\n",
            encoding="utf-8",
        )
        (workdir / "service.rs").unlink()

        changed_manifest = Path(tmp) / "changed.json"
        deleted_manifest = Path(tmp) / "deleted.json"
        changed_manifest.write_text(
            json.dumps({"files": ["container.rs", "service_added.rs"]}) + "\n",
            encoding="utf-8",
        )
        deleted_manifest.write_text(
            json.dumps({"files": ["service.rs"]}) + "\n", encoding="utf-8"
        )
        # KHÔNG clean giữa seed và incremental — cleanup phải xoá node thật
        # (File + functions/types của service.rs) khỏi graph seed.
        dual(driver, workdir, "inc_run", host, port, report,
             incremental=(changed_manifest, deleted_manifest), clean=False)


def count_stock_rs(stock: Path) -> int:
    count = 0
    for dirpath, dirnames, filenames in os.walk(stock):
        dirnames[:] = [name for name in dirnames if name not in RUST_SKIP_DIRS]
...
def main() -> int:
    ...
    if not RUST_BIN.exists():
        ...
        print("Build first: cargo build --release -p analyzer-rust (run inside rust/)")
        return 1
    ...
    report = [
        ...
        f"- testdata: `{TESTDATA.relative_to(REPO)}` (mod/impl/trait/generics/"
        "methods/alias/macro/cross-file use)",
    ...
    if not args.skip_stock and STOCK.exists():
        stock_rs = count_stock_rs(STOCK)
        if stock_rs >= 3:
            print(f"[stock] {stock_rs} .rs files scanned (excluding skip dirs)")
            dual(driver, STOCK, "stock_full", args.host, args.port, report)
        else:
            note = (
                f"[warn] skip stock: chỉ {stock_rs} file .rs dưới stock "
