...
# ── sync code ── (phase-06 sync-plane Rust cutover: the Python sync
# command bodies (:3400-3560) are DELETED — entrypoints are binary-only
# (cortex-dev); the python sync plane itself was deleted at the same
# cutover. Loud retired stubs keep the click group importable.

@sync.group("code", invoke_without_command=True)
@click.pass_context
def sync_code(ctx):
    """RETIRED (phase-08-style loud retired-error): sync runs via the
    cortex-dev binary (`dev sync code`); the Python orchestrator
    (code-tiny/tools/sync/incremental_sync.py) was deleted at the
    sync-plane Rust cutover (tag: pre-syncplane-delete)."""
    raise click.ClickException(
        "python sync plane retired at the sync-plane Rust cutover; "
        "use the cortex-dev binary — rollback = git revert (tag pre-syncplane-delete)"
    )
