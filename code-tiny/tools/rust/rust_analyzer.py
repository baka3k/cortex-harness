...
def _scan_rust_files(root: str) -> List[str]:
    paths: List[str] = []
...
def build_call_graph(root: str, output: Optional[str] = None) -> Dict[str, Any]:
    files = _scan_rust_files(root)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("path", help="Rust source file or directory")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if os.path.isdir(path):
        result = build_call_graph(path, args.output)
    else:
