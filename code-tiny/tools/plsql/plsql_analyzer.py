...
_PLSQL_IDENTIFIER = r"[A-Za-z_][\w$#]*"
_PLSQL_QUALIFIED_IDENTIFIER = rf"(?:{_PLSQL_IDENTIFIER}\.)*{_PLSQL_IDENTIFIER}"

_PLSQL_CREATE_RE = re.compile(
    rf"\bcreate\s+(?:or\s+replace\s+)?(?P<kind>procedure|proc|function)\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})",
    re.IGNORECASE,
)
_PLSQL_PACKAGE_BODY_RE = re.compile(
    rf"\bcreate\s+(?:or\s+replace\s+)?package\s+body\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})",
    re.IGNORECASE,
)
_PLSQL_PACKAGE_RE = re.compile(
    rf"\bcreate\s+(?:or\s+replace\s+)?package\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})",
    re.IGNORECASE,
)
_PLSQL_TRIGGER_RE = re.compile(
    rf"\bcreate\s+(?:or\s+replace\s+)?trigger\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})",
    re.IGNORECASE,
)
_PLSQL_PROC_RE = re.compile(rf"\bprocedure\s+(?P<name>{_PLSQL_IDENTIFIER})", re.IGNORECASE)
_PLSQL_FUNC_RE = re.compile(rf"\bfunction\s+(?P<name>{_PLSQL_IDENTIFIER})", re.IGNORECASE)
_PLSQL_CALL_RE = re.compile(rf"\bcall\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})", re.IGNORECASE)
_PLSQL_EXEC_RE = re.compile(rf"\bexec(?:ute)?\s+(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})", re.IGNORECASE)
_PLSQL_GENERIC_CALL_RE = re.compile(rf"\b(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})\s*\(", re.IGNORECASE)
_PLSQL_BARE_CALL_RE = re.compile(rf"^\s*(?P<name>{_PLSQL_QUALIFIED_IDENTIFIER})\s*;\s*$", re.IGNORECASE | re.MULTILINE)
_PLSQL_BODY_START_RE = re.compile(r"\b(as|is|begin)\b", re.IGNORECASE)
_PLSQL_JOB_CALL_RE = re.compile(r"\bdbms_scheduler\s*\.\s*create_job\s*\(", re.IGNORECASE)
_PLSQL_JOB_NAME_ARG_RE = re.compile(r"\bjob_name\s*=>\s*'(?P<value>(?:''|[^'])*)'", re.IGNORECASE | re.DOTALL)
_PLSQL_JOB_ACTION_ARG_RE = re.compile(r"\bjob_action\s*=>\s*'(?P<value>(?:''|[^'])*)'", re.IGNORECASE | re.DOTALL)
...
_PLSQL_BLOCK_END_LABELS: Set[str] = {
    "if",
    "loop",
    "case",
    "while",
...
_PLSQL_BUILTIN_PREFIXES: Tuple[str, ...] = (
    "dbms_",
    "utl_",
    "sys.",
    "sys_",
...
def _find_definition_end(masked_text: str, start_idx: int) -> int:
    for end_match in re.finditer(
        ...
        if label in _PLSQL_BLOCK_END_LABELS:
            continue
...
def _find_package_end(masked_text: str, start_idx: int, package_name: str) -> int:
    short_name = package_name.rsplit(".", 1)[-1]
    patterns = [
        rf"\bend\b\s+{re.escape(package_name)}\s*;",
        rf"\bend\b\s+{re.escape(short_name)}\s*;",
    ]
...
def _find_matching_paren(text: str, open_index: int) -> Optional[int]:
    depth = 0
    in_string: Optional[str] = None
    for idx in range(open_index, len(text)):
        ch = text[idx]
...
def _is_valid_callee(simple_name: str) -> bool:
    ...
    if not token:
        return False
    if token in _PLSQL_CALL_KEYWORDS:
        return False
    if token in _PLSQL_TYPE_KEYWORDS:
        return False
    return True
...
def _is_builtin_callee(qualified_name: str, simple_name: str) -> bool:
    ...
    if not s:
        return True
    if s in _PLSQL_CALL_KEYWORDS or s in _PLSQL_TYPE_KEYWORDS:
        return True
    if any(q.startswith(prefix) for prefix in _PLSQL_BUILTIN_PREFIXES):
        return True
    return False


def _extract_calls_from_body(body_masked: str, *, base_line: int = 0) -> List[Dict[str, Any]]:
    ...
    for matcher in (_PLSQL_CALL_RE, _PLSQL_EXEC_RE, _PLSQL_GENERIC_CALL_RE):
        for match in matcher.finditer(body_masked):
            append_call(match.group("name"), match.start())

    # Best-effort for no-parenthesis calls: e.g. `my_proc;`
    for match in _PLSQL_BARE_CALL_RE.finditer(body_masked):
        append_call(match.group("name"), match.start())
...
def _find_package_ranges(masked_text: str) -> List[Tuple[int, int, str]]:
    ...
    for match in _PLSQL_PACKAGE_BODY_RE.finditer(masked_text):
        ...
        end_idx = _find_package_end(masked_text, match.end(), name)
...
def _extract_job_blocks(source: str, masked: str, rel_path: str) -> Tuple[List[FunctionDef], List[CallEdge]]:
    functions: List[FunctionDef] = []
    calls: List[CallEdge] = []
    seen_ids: Set[str] = set()
    for match in _PLSQL_JOB_CALL_RE.finditer(masked):
        open_idx = source.find("(", match.start(), match.end() + 2)
        if open_idx == -1:
            continue
        close_idx = _find_matching_paren(source, open_idx)
        if close_idx is None:
            continue
        args_text = source[open_idx + 1 : close_idx]
        action_match = _PLSQL_JOB_ACTION_ARG_RE.search(args_text)
        if not action_match:
            continue
        action_sql = action_match.group("value").replace("''", "'")
        if not action_sql.strip():
            continue
        name_match = _PLSQL_JOB_NAME_ARG_RE.search(args_text)
        raw_name = name_match.group("value").replace("''", "'").strip() if name_match else ""
        job_name = raw_name or f"job_block_{_line_from_index(source, match.start())}"
        scope_name = "DBMS_SCHEDULER"
        symbol_id = _symbol_id(scope_name, job_name, 0, rel_path)
        if symbol_id in seen_ids:
            symbol_id = _symbol_id(scope_name, f"{job_name}_{_line_from_index(source, match.start())}", 0, rel_path)
        seen_ids.add(symbol_id)
        start_line = _line_from_index(source, match.start())
        end_line = _line_from_index(source, close_idx)
        comment = _extract_leading_comment_from_lines(source.splitlines(), start_line)
        summary = comment
        note = _build_note(action_sql, comment, summary)
        functions.append(
            FunctionDef(
                symbol_id=symbol_id,
                qualified_name=_qualified_name(scope_name, job_name),
                name=job_name,
                kind="job_block",
                scope_name=scope_name,
                file_path=rel_path,
        ...
        action_masked = _mask_plsql_comments(action_sql)
        for call_item in _extract_calls_from_body(action_masked, base_line=start_line):
            calls.append(
                CallEdge(
                    caller_id=symbol_id,
                    caller_scope=scope_name,
...
def parse_plsql_file(path: str, root: str) -> Tuple[
    ...
    def add_function(item: FunctionDef) -> bool:
        if item.symbol_id in function_ids:
            return False
        function_ids.add(item.symbol_id)
        functions.append(item)
        return True

    def add_relation(item: RelationEdge) -> None:
        key = (item.source_id, item.target_id, item.rel_type)
        if key in relation_keys:
            return
        relation_keys.add(key)
        relations.append(item)

    for start_idx, end_idx, pkg_name_raw in _find_package_ranges(masked):
        ...
        for matcher in (_PLSQL_PROC_RE, _PLSQL_FUNC_RE):
            for match in matcher.finditer(segment_masked):
                ...
                name = match.group("name")
                if segment_masked[max(0, local_start - 5) : local_start].lower().endswith("end "):
                    continue
                routine_end = _find_routine_end(masked, abs_start, name)
                if routine_end <= abs_start:
                    continue
                if routine_end > end_idx:
                    routine_end = end_idx
                ...
                func_id = _symbol_id(pkg_scope, name, arity, rel_path)
                kind = "procedure" if matcher is _PLSQL_PROC_RE else "function"
                if not add_function(
                    FunctionDef(
                        symbol_id=func_id,
                        qualified_name=_qualified_name(pkg_scope, name),
                        name=name,
                        kind=kind,
                        scope_name=pkg_scope,
                        file_path=rel_path,
                ...
                for call_item in _extract_calls_from_body(body_masked, base_line=body_start_line):
                    calls.append(
                        CallEdge(
                            caller_id=func_id,
                            caller_scope=pkg_scope,
                            callee_name=call_item["callee_simple"],
                            callee_id=None,
                            callee_arity=None,
                            callee_raw=call_item["callee_raw"],
    ...
    for match in _PLSQL_TRIGGER_RE.finditer(masked):
        full_name = match.group("name")
        scope_name, name = _split_scope(full_name)
        start_idx = match.start()
        end_idx = _find_routine_end(masked, match.end(), full_name)
        ...
        summary = comment
        note = _build_note(snippet, comment, summary)
        func_id = _symbol_id(scope_name, name, 0, rel_path)
        if not add_function(
            FunctionDef(
                symbol_id=func_id,
                qualified_name=_qualified_name(scope_name, name),
    ...
    job_functions, job_calls = _extract_job_blocks(source, masked, rel_path)
...
def _should_ignore_directory(dir_name: str, dir_path: str) -> bool:
    """
    Check if a directory should be ignored during PL/SQL project scanning.

    Ignores:
    - Oracle outputs: output/, logs/, .sql_history
    - Database backups: *.bak, *.backup, *.dmp, *.exp
    - Compiled PL/SQL: *.plb (optional)
    """
    ignore_patterns = {
        # Version control
        ".git", ".svn", ".hg",
        ...
        # Oracle specific outputs
        "output", "logs", ".sql_history", "log",

        # Backup directories
        "backup", "backups", "exports", "dumps",
...
def _scan_plsql_files(root: str) -> List[str]:
    """
    Scan for PL/SQL files, ignoring unnecessary directories.
    """
    files: List[str] = []
    ...
    for dirpath, dirnames, filenames in os.walk(root, topdown=True):
        ...
        for name in filenames:
            # Skip compiled and backup files
            if name.endswith((".plb", ".bak", ".backup", ".dmp", ".exp", ".gz")):
                continue
            if name.endswith((".swp", ".swo", ".log")):
                continue
            if name in (".DS_Store", "Thumbs.db", ".sql_history"):
                continue

            if name.endswith(
                (".sql", ".pls", ".plsql", ".pks", ".pkb", ".pkg", ".pck", ".spc", ".spb", ".trg", ".fnc")
            ):
...
async def build_call_graph(
    ...
    cache_root = safe_cache_root(cache_dir, "plsql_analyzer", project_root=root)
    ...
    all_scanned_files = _scan_plsql_files(root)
    ...
    if verbose:
        ...
        print(f"[scan] Found {len(selected_files)} PL/SQL files under {root}")
                ...
                if should_materialize_external(call):
                    ...
                    if symbol_id not in known_function_ids and symbol_id not in external_functions:
                        ...
                        external_functions[symbol_id] = {
                            ...
                            "code": "",
                            "comment": "",
                            "summary": "External callee inferred from PL/SQL callsite",
                            "note": "",
                            "exported": False,
                            "external": True,
                            "builtin": is_builtin,
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PL/SQL call graph analyzer")
    parser.add_argument("--root", required=True, help="Root folder containing PL/SQL sources")
    ...
    parser.add_argument("--batch-size", type=int, default=int(os.environ.get("EMBED_BATCH_SIZE", 8)))  # for embedding - 4 function 1 turn embedding
...
async def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.ignore_cache:
        run_cache_root = safe_cache_root(effective_cache_dir, "plsql_analyzer", project_root=args.root)
    ...
    if not args.disable_neo4j_resume and not args.incremental:
        cache_root = safe_cache_root(effective_cache_dir, "plsql_analyzer", project_root=args.root)
    ...
    try:
        if args.dry_run:
            ...
            if args.incremental and changed_manifest_files:
                ...
                print(
                    "Dry run (incremental): %d PL/SQL files selected (manifest=%d)"
