...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="ASP.NET Framework semantic overlay analyzer", allow_abbrev=False,
    )
    add_shared_arguments(parser, output_flag="--aspnet-framework-preview-output")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not args.quiet:
        print(
            "[aspnet_framework] modules=%d facts=%d relationships=%d diagnostics=%d coverage=%s"
    ...
    try:
        ...
        print(f"[aspnet_framework] graph write failed: {exc}", file=sys.stderr)
        return 3
    if args.verbose:
        print(f"[aspnet_framework] graph={summary}")
    return 0
