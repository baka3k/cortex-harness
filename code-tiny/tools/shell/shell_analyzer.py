#!/usr/bin/env python3
"""CLI entry point for the shell (`.sh`) batch-script analyzer.

Follows the argparse/graph-persistence conventions of `tools/go/go_analyzer.py`:
`--root`, `--dry-run`, standard graph provider args, and the shared
`LanguageCodeWriter`/vector-sync helpers. Persists scripts as `File` nodes,
functions as `Function` nodes, and config-read/call-edge facts as generic
`RelationEdge` rows (no new writer methods required).
"""
...
from tools.shell.models import SUPPORTED_EXTENSIONS
...
def _scan_shell_files(root: str) -> List[str]:
    matches: List[str] = []
...
def analyze_root(root: str, *, selected_rel_paths: Optional[List[str]] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for path in _scan_shell_files(root):
        ...
        files_rows.append(
            {
                ...
                "parse_meta": {
                    "source_encoding": script.source_encoding,
                    "source_encoding_lossy": script.source_encoding_lossy,
                    "variable_count": len(script.variables),
                    "config_read_count": len(script.config_reads),
                    "call_edge_count": len(script.call_edges),
                },
            }
        )
        ...
        for relation in build_relations(script):
    ...
    return {"files": files_rows, "functions": function_rows, "relations": relation_rows}
...
def _sync_vectors(args: argparse.Namespace, rows: Dict[str, List[Dict[str, Any]]]) -> int:
    if not vector_configured(args.qdrant_url):
        return 0
    documents = documents_from_rows(rows, parser="shell", root_scope=args.repo or "", max_chars=args.max_embed_chars)
    cleanup_targets = sorted(
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="POSIX shell (.sh) batch-script analyzer")
    parser.add_argument("--root", required=True, help="Root directory to scan for .sh files")
    ...
    parser.add_argument("--qdrant-collection", default=os.environ.get("QDRANT_COLLECTION", "shell_functions"))
...
async def main(argv: Optional[List[str]] = None) -> int:
            ...
            json.dumps(
                {
                    ...
                    "function_count": len(rows["functions"]),
    ...
    try:
        ...
        print(f"Shell graph persistence failed: {exc}", file=sys.stderr)
        return 3
    ...
    try:
        ...
        print(f"Shell vector persistence failed: {exc}", file=sys.stderr)
        return 4

    print(
        f"[SCAN_RESULT] parser=shell files={len(rows['files'])} functions={len(rows['functions'])} "
