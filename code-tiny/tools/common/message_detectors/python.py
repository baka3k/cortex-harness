...
_PY_FN_PATTERN = re.compile(r"^\s*(?:async\s+)?def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(")


class PythonMessageDetector(BaseMessageDetector):
    parser_name = "python"
    keywords: Set[str] = {
        "publish",
        "send",
    ...
    def extract_sender(self, line_text: str) -> Optional[str]:
        match = _PY_FN_PATTERN.match(line_text.strip())
    ...
    def extract_fields(self, callee_name: str, args: Sequence[str]):
        ...
        explanation = f"{callee_name}() inferred by python detector"
