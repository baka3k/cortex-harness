...
_VBSCRIPT_FN_PATTERN = re.compile(r"^\s*(?:Sub|Function)\s+([A-Za-z_][A-Za-z0-9_]*)", re.IGNORECASE)


class VbScriptMessageDetector(BaseMessageDetector):
    parser_name = "vbscript"
    keywords: Set[str] = {
        "publish",
        "send",
        "notify",
    ...
    def extract_sender(self, line_text: str) -> Optional[str]:
        match = _VBSCRIPT_FN_PATTERN.match((line_text or "").strip())
    ...
    def extract_fields(self, callee_name: str, args: Sequence[str]):
        ...
        explanation = f"{callee_name}() inferred by vbscript detector"
