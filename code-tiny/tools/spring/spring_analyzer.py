...
from tools.spring.pipeline import run_spring_foundation


@dataclass(frozen=True)
class SpringAnalyzerConfig:
    root: str
    project_id: str
    project_name: str
...
class SpringAnalyzer:
    def __init__(self, config: SpringAnalyzerConfig) -> None:
        self.config = config

    def analyze(self):
        return run_spring_foundation(
            root=self.config.root,
            project_id=self.config.project_id,
            project_name=self.config.project_name,
            languages=self.config.languages,
            incremental=self.config.incremental,
            changed_files_manifest=self.config.changed_files_manifest,
            deleted_files_manifest=self.config.deleted_files_manifest,
        )


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Spring semantic foundation analyzer", allow_abbrev=False)
    ...
    parser.add_argument("--spring-facts-output", default="")
...
async def _write_graph(args: argparse.Namespace, result) -> int:
    ...
    if provider == "falkordb":
        ...
        if not creds_complete:
            if require_neo4j:
                print(
                    "[spring] ERROR: --require-neo4j is on but Neo4j credentials are incomplete",
                    file=sys.stderr,
                )
                return 2
            return 0
        try:
            ...
            print(f"[spring] ERROR: Neo4j driver creation failed: {exc!r}", file=sys.stderr)
            return 3
    try:
        writer = SpringFactWriter(driver, database=database, batch_size=args.neo4j_batch_size, verbose=args.verbose)
        ...
        print(f"[{provider_label}] spring_facts {written}/{len(result.semantic_facts)}")
        rel_written = await writer.write_relationships([rel.to_graph_row() for rel in result.relationships])
        print(f"[{provider_label}] spring_relationships {rel_written}/{len(result.relationships)}")
    finally:
        await _close_driver(driver)
    return 0
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    artifact_path = args.spring_facts_output or default_fact_artifact_path(args.cache_dir, root, project_id)
    write_fact_artifact(artifact_path, result)
    print(
        "[spring] modules=%d configs=%d language_facts=%d semantic_facts=%d relationships=%d diagnostics=%d artifact=%s"
        % (
            len(result.modules),
            len(result.config_values),
            len(result.language_facts),
