...
def resolve_mybatis_relationships(
    *,
    ...
    for mapper in mapper_interfaces:
        relationships.append(_rel(mapper.stable_id, mapper.java_class_symbol_id, "SEMANTIC_OF", project_id, mapper.source, reason="mapper interface anchor"))
        for method in [item for item in mapper_methods if item.mapper_fqcn == mapper.fqcn]:
            relationships.append(_rel(mapper.stable_id, method.stable_id, "DECLARES_METHOD", project_id, method.source, reason="mapper method declaration"))
            if method.java_symbol_id:
                relationships.append(_rel(method.stable_id, method.java_symbol_id, "SEMANTIC_OF", project_id, method.source, reason="mapper method Java anchor"))
    ...
    for stmt in statements:
        ...
        if mapper is None:
            ...
            relationships.append(_rel(mapper.stable_id, stmt.stable_id, "DECLARES_STATEMENT", project_id, stmt.source, reason="statement namespace matches mapper"))

        candidates = methods_by_mapper_name.get((stmt.namespace, stmt.statement_id), [])
        statement_bindings[stmt.stable_id] = candidates
        if len(candidates) == 1:
            relationships.append(_rel(candidates[0].stable_id, stmt.stable_id, "BINDS_STATEMENT", project_id, stmt.source, reason="method name matches statement id"))
            ...
            for candidate in candidates:
                relationships.append(_rel(candidate.stable_id, stmt.stable_id, "BINDS_STATEMENT", project_id, stmt.source, resolution_status="ambiguous", reason="overloaded method name matches statement id", properties={"candidate_count": len(candidates)}))
        else:
    ...
    for table in sql_tables:
        ...
        relationships.append(_rel(table.sql_statement_id, table.stable_id, rel_type, project_id, table.source, resolution_status=table.resolution_status, reason=f"SQL table role {table.role}", properties={"role": table.role, "alias": table.alias}))
    for column in sql_columns:
        relationships.append(_rel(column.sql_statement_id, column.stable_id, "REFERENCES_COLUMN", project_id, column.source, resolution_status=column.resolution_status, reason=f"SQL column role {column.role}", properties={"role": column.role, "qualifier": column.qualifier}))
    for join in sql_joins:
        relationships.append(_rel(join.sql_statement_id, join.stable_id, "JOINS_WITH", project_id, join.source, resolution_status=join.resolution_status, reason="SQL join", properties={"join_type": join.join_type, "right_table": join.right_table, "right_alias": join.right_alias}))
...
def _resolve_statement_result_maps(project_id: str, stmt: MyBatisStatementFact, result_maps_by_ref: Dict[str, MyBatisResultMapFact], relationships: List[MyBatisRelationship], diagnostics: List[Diagnostic]) -> None:
    ...
    for ref in [item.strip() for item in result_map_attr.split(",") if item.strip()]:
        ...
        if target is None:
            ...
            continue
        relationships.append(_rel(stmt.stable_id, target.stable_id, "USES_RESULT_MAP", project_id, stmt.source, reason="statement resultMap attribute"))
...
def _resolve_parameters(project_id: str, sql_parameters: Sequence[MyBatisSqlParameterFact], statement_bindings: Dict[str, List[MyBatisMapperMethodFact]], params_by_method: Dict[str, List[MyBatisMapperParameterFact]], relationships: List[MyBatisRelationship], diagnostics: List[Diagnostic]) -> None:
    for sql_param in sql_parameters:
        ...
        for method in methods:
            ...
            relationships.append(_rel(sql_param.sql_statement_id, match.stable_id, "DEPENDS_ON_PARAMETER", project_id, sql_param.source, resolution_status=status, reason="SQL placeholder resolved to mapper parameter", properties={"sql_parameter_id": sql_param.stable_id, "parameter_name": sql_param.name, "parameter_kind": sql_param.parameter_kind}))
    ...
    for result_map in result_maps:
        if result_map.extends:
            ...
            if target is None:
                ...
                relationships.append(_rel(result_map.stable_id, target.stable_id, "EXTENDS_RESULT_MAP", project_id, result_map.source, reason="resultMap extends"))
    for mapping in result_mappings:
        result_map = maps_by_id.get(mapping.result_map_id)
        relationships.append(_rel(mapping.result_map_id, mapping.stable_id, "HAS_RESULT_MAPPING", project_id, mapping.source, reason="result-map mapping member"))
        if result_map is not None and mapping.property_name:
            ...
            if prop is not None:
                relationships.append(_rel(mapping.stable_id, prop.stable_id, "MAPS_PROPERTY", project_id, mapping.source, reason="result mapping property matches Java property"))
            ...
            for sql_statement_id in statements_using_map.get(result_map.stable_id, []):
                for column in columns_by_statement.get(sql_statement_id, []):
                    if column.role == "projection" and column.normalized_name == mapping.column.lower():
                        relationships.append(_rel(mapping.stable_id, column.stable_id, "MAPS_COLUMN", project_id, mapping.source, resolution_status=column.resolution_status, reason="result mapping column matches SQL column", properties={"sql_statement_id": sql_statement_id}))
        if mapping.nested_select and result_map is not None:
            ...
            if target_stmt is not None:
                relationships.append(_rel(mapping.stable_id, target_stmt.stable_id, "NESTED_SELECT", project_id, mapping.source, reason="nested select mapping"))
        if mapping.nested_result_map and result_map is not None:
            ...
            if target_map is not None:
                ...
                relationships.append(_rel(mapping.stable_id, target_map.stable_id, rel_type, project_id, mapping.source, reason="nested resultMap mapping"))
...
def _rel(from_id: str, to_id: str, rel_type: str, project_id: str, source, *, confidence: float = 1.0, resolution_status: str = "resolved", reason: str = "", properties: Dict[str, object] | None = None) -> MyBatisRelationship:
