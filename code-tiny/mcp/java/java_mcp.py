...
async def _get_graph_driver() -> GraphDriver:
    ...
    if DEFAULT_GRAPH_PROVIDER == "falkordb":
        ...
        if not remote_uri:
            ...
            # under the configured ``CORTEX_DATA_HOME``; see cplus_mcp.
...
async def _list_relationship_types(dbs: List[str]) -> List[str]:
    # Provider-neutral introspection: FalkorDB, LadybugDB, and Neo4j each get
    # their native dialect inside the driver.
    try:
        driver = await _get_graph_driver()
...
async def tool_semantic_search(
    ...
    if mode == "comment":
        ...
        if _search_timing_enabled():
            logger.info(
                "semantic_search timing[java]: embed=%.3f resolve=%.3f qdrant=%.3f expand=%.3f total=%.3f",
...
async def tool_trace_flow(
    ...
    if end_id is not None:
        ...
        used_db, result = await _run_cypher_first(query, {"start": start_id, "limit": limit, "project_id": project_id}, candidates)
        if not result:
            return {"db": used_db, "nodes": [], "edges": [], "direction": direction, "rel_types": resolved_rel_types, "max_depth": depth, "reason": "no_path"}
        paths = [row["p"] for row in result]
...
async def tool_trace_flow_between_module(
    ...
    used_db, results = await _run_cypher_first(
        query, {"sources": source_modules, "targets": target_modules, "limit": limit, "project_id": project_id}, db_candidates
    )
    if not results and direction not in {"both", "any", "undirected"}:
        rel_match_both = _build_rel_match(resolved_rel_types, depth, "both")
        fallback_query = query.replace(f"(s){rel_match}(t)", f"(s){rel_match_both}(t)")
        used_db, results = await _run_cypher_first(
            fallback_query, {"sources": source_modules, "targets": target_modules, "limit": limit, "project_id": project_id}, db_candidates
        )
    paths = [row["p"] for row in results]
    graph = _paths_to_graph(paths, content_mode=content_mode or "auto", include_raw_fields=include_raw_fields)
    graph["db"] = used_db
...
_JAVA_PARAMETER_GUIDELINES: Dict[str, Any] = {
    "always_call_first": "list_mcp_functions",
    "rules": [
        "Use exact parameter names documented in tool inputs.",
        "Provide all required params explicitly.",
        "Prefer array values for list-like parameters.",
        "On invalid_parameters/missing_required_parameters, fix arguments and retry.",
    ],
}
_MCP_FUNCTIONS_JSON: str = json.dumps(
    {
        "total_count": len(_java_catalog),
        "parameter_guidelines": _JAVA_PARAMETER_GUIDELINES,
        "functions": _java_catalog,
    },
    ensure_ascii=False,
)
