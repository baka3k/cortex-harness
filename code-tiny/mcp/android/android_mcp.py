...
async def _get_graph_driver() -> GraphDriver:
    ...
    if DEFAULT_GRAPH_PROVIDER == "falkordb":
        ...
        if not remote_uri:
            # Boot path: per-instance isolation (Phase 02). See cplus_mcp
            # for the rationale; sibling reads are gated for tool-level
            # callers via ``code_tiny.mcp.cross_instance``.
...
_ANDROID_PARAMETER_GUIDELINES: Dict[str, Any] = {
...
_MCP_FUNCTIONS_JSON: str = json.dumps(
    {
        "total_count": len(_android_catalog),
        "parameter_guidelines": _ANDROID_PARAMETER_GUIDELINES,
        "functions": _android_catalog,
    },
    ensure_ascii=False,
)
