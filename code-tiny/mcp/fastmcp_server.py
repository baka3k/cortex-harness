...
# Module-level state was removed as part of the unified ingest/query contract
# plan (Phase 02d). Every project-scoped tool now resolves its graph through
# ``unified_mcp.ProjectTargets`` and accepts ``project_id`` per call. Omit
# ``project_id`` to span every project. There is no stateful default.
...
async def tool_semantic_search(
    ...
    if mode == "comment":
        ...
        if _search_timing_enabled():
            logger.info(
                "semantic_search timing[fast]: embed=%.3f resolve=%.3f qdrant=%.3f expand=%.3f total=%.3f",
...
async def tool_list_workflows(
    ...
    if not db_candidates:
        db_candidates = [DEFAULT_GRAPH_DB]
    filters = []
    ...
    if project_id is not None:
        filters.append("w.project_id_normalized STARTS WITH $project_id_normalized")
...
async def tool_search_workflows(
    ...
    WHERE (toLower(w.name) CONTAINS toLower($q)
      ...
      AND ($project_id IS NULL OR w.project_id_normalized STARTS WITH $project_id_normalized)
    RETURN w.workflow_id   AS workflow_id,
           w.name          AS name,
           w.domain        AS domain,
           w.description   AS description,
